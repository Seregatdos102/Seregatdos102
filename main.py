...
МАКСИМАЛЬНЫЙ АНТИФРОД, ЗАЩИТА ОТ БЛОКИРОВОК
...
class AntiFraudEngine:
    ...
    def check_rate_limits(self, acc_id: str) -> bool:
        ...
        if counters["today_actions"] >= settings.max_actions_per_day:
            self.logger.warning(acc_id, f"Daily limit exceeded: {counters['today_actions']}")
            return False
        
        if counters["hour_actions"] >= settings.max_actions_per_hour:
            self.logger.warning(acc_id, f"Hourly limit exceeded: {counters['hour_actions']}")
            return False
    ...
    def record_action(self, acc_id: str, action_type: str, success: bool = True):
        ...
        # Оставляем толь��о последние 100 действий
    ...
    def _detect_unnatural_pattern(self, acc_id: str) -> bool:
        ...
        if len(intervals) >= 4:
            ...
            if variation < avg_interval * 0.1:  # Слишком мало вариации
                self.logger.warning(acc_id, "Detected unnatural pattern: perfect regularity")
                return True
...
{Fore.LIGHTGREEN_EX}💡 🛡️ АНТИФРОД: Адаптивные delays, Rate Limits, Ban Detection!{Style.RESET_ALL}
{Fore.CYAN}{'=' * 90}{Style.RESET_ALL}
""")
